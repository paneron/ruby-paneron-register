...
RSpec.describe Paneron::Register::Raw::RegisterRoot do
  it "detects invalid register path on initialization" do
    expect do
      Paneron::Register::Raw::RegisterRoot.new("spec/fixtures/doesnotexist")
    end.to raise_error Paneron::Register::Error
  end

  it "allows valid register path on initialization" do
    root =
      Paneron::Register::Raw::RegisterRoot.new("spec/fixtures/test-register")
    expect(root).to be_instance_of Paneron::Register::Raw::RegisterRoot
  end
  ...
  describe "with a valid register root repository" do
    ...
    it "retrieves register objects" do
      root.registries.each_pair do |_register_name, register|
        expect(register).to be_instance_of(Paneron::Register::Raw::Register)
      end
    end
